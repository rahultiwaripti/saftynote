...
#/api/employee/:id/lovedones
  def getlovedones
    ...
    @employee = Employee.find(employee_id)
    @employee.update(:latitude => params[:latitude], :longitude  => params[:longitude])
    @providers = @employee.company.providers
    @lovedones = Lovedone.where(:id => @providers.collect(&:lovedone_id)).order(:last_name)
  ...
  #/api/trip/new    
    ...
    def updatetrip
      ...
      #curent_emp_id=current_user.company.providers.id
      last_emp_id = lovedone.employee_id

       render json: curent_emp_id, status: :ok
       return false 
  ...
  private
    ...
    def send_emailer(employee, lovedone)
      ...
      approved_followers.each do |follower|
       ...
       # if(follower.user.profile.contact_method==nil)
       # return false
       # end 
        ...
        #unless follower.user.profile.contact_method.  ('email')==nil
          track_mail.deliver
          sponsor_id = 0
          sponsor_id = sponsor.id unless sponsor.id.nil?
          begin
          ...
          sponsor = Sponsor.next(sponsor.id) unless sponsor.id.nil?
        #end

        
        #unless follower.user.profile.contact_method.index('text')==nil
