class RegistrationsController < Devise::RegistrationsController
	...
	def create
		...
		if params[:query] == "serviceprovider"
			...
			unless resource.id == nil
				company = Company.new sign_up_params_for_company[:company_attributes]
				company.user_id = resource.id
				if company.save
					SystemMailer.receiveapp_email(resource.email).deliver
				end
    ...
    params.require(:user).permit(:name, :email, :password, :password_confirmation, :terms, 
    	company_attributes: [:company_name, :business_name, :address, :city, :county, :state, :county, :zip, :telephone, 
    		:mobile_phone_number, :contact_last_name, :contact_first_name, :provider_type, :get_notification, {:contact_method => []}]
    	)
  end
  ...
  def sign_up_params_for_lovedone
    ...
    #lovedones_attributes
    params.require(:user).permit(
    	lovedones_attributes: [:last_name, :first_name, :middle_initial, :nick_name, :date_of_birth, :gender, :city, :county, :state]
    	)
  end
